...
while (-not $salir) {
    ...
    # ---- Obtener datos de red de forma robusta ----
    # Metodo 1: buscar adaptador con IP asignada activa
    $activeAdapter = $null
    $ipConfig      = $null

    $candidatos = Get-NetAdapter | Where-Object { $_.Status -eq "Up" }

    foreach ($adaptador in $candidatos) {
        $cfg = Get-NetIPAddress -InterfaceIndex $adaptador.ifIndex `
                                -AddressFamily IPv4 -ErrorAction SilentlyContinue |
               Where-Object { $_.IPAddress -notlike "169.254.*" } |
               Select-Object -First 1
        if ($cfg) {
            $activeAdapter = $adaptador
            $ipConfig      = $cfg
            break
        }
    }

    # Metodo 2: fallback usando Get-NetIPConfiguration (mas tolerante)
    if ($null -eq $activeAdapter) {
        $cfgFallback = Get-NetIPConfiguration | Where-Object {
            $_.IPv4Address -and
            $_.IPv4Address.IPAddress -notlike "169.254.*"
        } | Select-Object -First 1

        if ($cfgFallback) {
            $activeAdapter = Get-NetAdapter -InterfaceIndex $cfgFallback.InterfaceIndex -ErrorAction SilentlyContinue
            $ipConfig      = $cfgFallback.IPv4Address | Select-Object -First 1
        }
    }

    # Asignar valores finales con fallback a "No disponible"
    if ($activeAdapter) {
    ...
    $ip = if ($ipConfig -and $ipConfig.IPAddress) { $ipConfig.IPAddress } else { "No asignada" }
